...
import type { CustomTypesClientMethodParams } from "../src"
import { InvalidPayloadError, NotFoundError } from "../src"
...
it("throws NotFoundError if a matching slice was not found", async ({ client, api, slice }) => {
	api.mock("./slices/update", undefined, {
		method: "post",
		statusCode: 422,
		requiredBody: slice,
...
it("throws InvalidPayloadError if an invalid slice is sent", async ({ client, api, slice }) => {
	api.mock("./slices/update", undefined, {
		method: "post",
		statusCode: 400,
		requiredBody: slice,
...
testFetchOptions("supports fetch options", {
	...
	mockURLMethod: "post",
	run: (client, params) => client.updateSharedSlice({} as prismic.SharedSliceModel, params),
})
