...
import type { CustomTypesClient } from "../../src"
...
export const it = test.extend<Fixtures>({
	client: async ({ task }, use) => {
		const repositoryName = crypto.createHash("md5").update(task.name).digest("hex")
	...
	// oxlint-disable-next-line no-empty-pattern -- Vitest fixture signature requires destructuring
	msw: async ({}, use) => {
		await use(server)
		...
		await use({
			mock: (path, response, options) => {
				...
				const handler = http[method](
					path instanceof URL ? path.toString() : new URL(path, client.endpoint).toString(),
					async ({ request }) => {
						if (
							request.headers.get("Authorization") !== `Bearer ${options?.token ?? client.token}` ||
						...
						if (options?.requiredSearchParams) {
							for (const name in options.requiredSearchParams) {
								...
								if (url.searchParams.get(name) !== options.requiredSearchParams[name]) {
									return
								}
							}
						}
						...
						if (options?.requiredBody) {
							if (JSON.stringify(await request.json()) !== JSON.stringify(options.requiredBody)) {
								return
							}
						}
