...
This package is primarily maintained by [Prismic](https://prismic.io)[^1]. External contributions are welcome. Ask for help by [opening an issue](https://github.com/prismicio/prismic-custom-types-client/issues/new/choose), or request a review by opening a pull request.
...
- All API network requests are called through the private `CustomTypesClient.prototype.fetch` method.

#### TSDocs

- All public APIs should have [TSDoc](https://tsdoc.org/) comments.
- TSDoc comments should be clear and concise. They should give users just enough context to understand how and when to use an API.

#### Write tests

- Any change to this library should be tested. A test can be as simple as ensuring a method calls the correct endpoint.
- Tests should be placed in the `test` directory using the existing file name convention.
- Tests are written using Vitest.
- [Fixtures](https://vitest.dev/guide/test-context) are provided using the custom `it` function from `test/__testutils__/it.ts`. See `it.ts` for a list of provided fixtures.

## :construction_worker: Develop

> [!NOTE]
> It's highly recommended to discuss your changes with the Prismic team before starting by [opening an issue](https://github.com/prismicio/prismic-custom-types-client/issues/new/choose).[^2]
>
> A short discussion can accellerate your work and ship it faster.

```sh
# Clone and prepare the project.
git clone git@github.com:prismicio/prismic-custom-types-client.git
cd prismic-custom-types-client
npm install
...
npm install @prismicio/custom-types-client@pr-101
...
This repository uses [Release Please](https://github.com/googleapis/release-please). To publish changes in `main`, merge [the pending Release Please PR](https://github.com/prismicio/prismic-custom-types-client/pulls?q=is%3Apr+is%3Aopen+label%3A%22autorelease%3A+pending%22).
